...
```bash
s3-lambda-workflow$ sam build --use-container
```
...
```bash
s3-lambda-workflow$ sam logs -n StockCheckerFunction --stack-name s3-lambda-workflow --tail
```
...
```bash
s3-lambda-workflow$ pip install -r tests/requirements.txt --user
# unit test
s3-lambda-workflow$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
s3-lambda-workflow$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
aws cloudformation delete-stack --stack-name s3-lambda-workflow
```
